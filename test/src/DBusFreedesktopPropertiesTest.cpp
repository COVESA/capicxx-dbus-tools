...
#include <commonapi/tests/EnumTypes.hpp>
...
TEST_F(FreedesktopPropertiesTest, CanSendAndReceiveNotificationForEnumProperty) {
    auto& testAttribute = proxy_->getEnumAttributeAttribute();

    bool callbackArrived = false;
    bool initialCall = true;

    std::function<void(const ::commonapi::tests::EnumTypes::e1)> listener([&](const ::commonapi::tests::EnumTypes::e1 value) {
        // the first call is for the initial value. Ignore it.
        if (initialCall) {
            initialCall = false;
        } else {
            bool isSame = (value == ::commonapi::tests::EnumTypes::e1::LastValue);
    ...
    CommonAPI::CallStatus callStatus = CommonAPI::CallStatus::REMOTE_ERROR;
    ::commonapi::tests::EnumTypes::e1 value;
    ::commonapi::tests::EnumTypes::e1 newValue = ::commonapi::tests::EnumTypes::e1::LastValue;
