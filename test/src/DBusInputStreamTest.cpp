...
TEST_F(InputStreamTest, ReadsBytes) {
    ...
    EXPECT_EQ(numOfElements, size_t(scopedMessage.getBodyLength()));
    for (unsigned int i = 0; i < numOfElements; i += 1) {
        uint8_t readVal;
        inStream >> readVal;
...
TEST_F(InputStreamTest, ReadsBools) {
    ...
    EXPECT_EQ(numOfElements*4, size_t(scopedMessage.getBodyLength()));
    for (unsigned int i = 0; i < numOfElements; i += 2) {
        bool readVal;
        inStream >> readVal;
        EXPECT_EQ(t, readVal);
...
TEST_F(InputStreamTest, ReadsUint16) {
    ...
    EXPECT_EQ(numOfElements*2, size_t(scopedMessage.getBodyLength()));
    for (unsigned int i = 0; i < numOfElements; i += 1) {
        uint16_t readVal;
        inStream >> readVal;
        EXPECT_EQ(val, readVal);
...
TEST_F(InputStreamTest, ReadsUint64) {
    ...
    EXPECT_EQ(numOfElements*8, size_t(scopedMessage.getBodyLength()));
    for (unsigned int i = 0; i < numOfElements; i += 1) {
        uint64_t readVal;
        inStream >> readVal;
        EXPECT_EQ(val, readVal);
...
TEST_F(InputStreamTest, ReadsStrings) {
    ...
    EXPECT_EQ(numOfElements*4 + numOfElements*4, size_t(scopedMessage.getBodyLength()));
    for (unsigned int i = 0; i < numOfElements; i += 1) {
        std::string readVal;
        inStream >> readVal;
        EXPECT_EQ(val, readVal);
...
TEST_F(InputStreamTest, ReadsArrays) {
    ...
    EXPECT_EQ(size_t(scopedMessage.getBodyLength()), numOfElements*4 + 4);
...
TEST_F(InputStreamTest, ReadsArraysInArrays) {
    ...
    EXPECT_EQ(numOfElements*numOfElements*4 + numOfElements*4 + 4, size_t(scopedMessage.getBodyLength()));
...
TEST_F(InputStreamTest, ReadsStringVariants) {
    ...
    EXPECT_EQ(numOfElements * (1+3+4+fromString.length()+1) + (numOfElements - 1) * (8-((fromString.length()+1)%8)) , size_t(scopedMessage.getBodyLength()));
    for (unsigned int i = 0; i < numOfElements; i += 1) {
        TestedVariantType readVariant;
        inStream.readValue(readVariant, static_cast<CommonAPI::DBus::VariantDeployment<CommonAPI::EmptyDeployment, CommonAPI::EmptyDeployment, CommonAPI::EmptyDeployment>*>(nullptr));
